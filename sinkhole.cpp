...
int main() {
	...
	int dp[1001][1001]; /* dp[i][j] = stores how many sinkholes it found in the submatrix region from top-left(0,0) to bottom-right(i,j) */
...
bool f(grid,k){
	...
	for(int i=0;i<=n-k;i++){
		for(int j=0;j<=m-k;j++){
			int val=dp[i+k][j+k]-dp[i][j+k]-dp[i+k][j]+dp[i][j];
			if(val<=1 && val<best){
				best=val;
				xt=i,xb=i+k,yl=j,yr=j+k;
			}
		}
	}
	return best<=1;
